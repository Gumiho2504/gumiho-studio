<template>
  <nav class="navbar">
    <div
      style="max-width:1280px;margin:0 auto;padding:0 1.5rem;display:flex;align-items:center;justify-content:space-between;height:68px;">
      <!-- Logo -->
      ...
      <!-- Desktop links -->
      <div style="" class="desktop-links flex items-center gap-6">
      ...
      <!-- Mobile hamburger -->
      <button class="mobile-ham" @click="mobileOpen = !mobileOpen"
        style="background:none;border:none;color:#f0eaff;cursor:pointer;padding:.3rem;" aria-label="Toggle menu">
    ...
    <Transition name="slide-down">
      <div v-if="mobileOpen"
        style="background:rgba(6,6,18,.97);backdrop-filter:blur(20px);border-bottom:1px solid rgba(176,74,255,.2);padding:1rem 1.5rem 1.5rem;">
        <div v-for="link in navLinks" :key="link.to"
          style="padding:.75rem 0;border-bottom:1px solid rgba(176,74,255,.1);">
          <RouterLink :to="link.to" class="nav-link" :class="{ active: isActive(link.to) }" @click="mobileOpen = false">
            {{ link.label }}
          </RouterLink>
        </div>
        <div style="margin-top:1rem;">
          <RouterLink to="/contact" class="btn-primary" style="width:100%;justify-content:center;"
